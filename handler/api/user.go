...
func (u *userAPI) Login(c *gin.Context) {
	...
	token, result := u.userService.Login(&model.User{
		Email:    user.Email,
		Password: user.Password,
	})
...
func (u *userAPI) GetUserTaskCategory(c *gin.Context) {
	taskCategories, result := u.userService.GetUserTaskCategory()
	if result != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "error retrieving user task categories"})
		return
	}
